...
if args.option == 'significance':
    ...
    base_path = 'eval/__results_revision/'
    base_path = Path(base_path)

    # ours_fn = base_path / f'{args.dataset}_rwnetav_bootstrap_results.json'
    ours_fn = '__results_revision/HRF/UNet++minmax0.05_bootstrap_results_5000_iterations.json'
    with open(ours_fn, 'r') as f:
        ours = json.load(f)
    # sota_fn = base_path / f'{args.dataset}_{args.sota}_bootstrap_results.json'
    sota_fn = '__results_revision/HRF/HRF_UNet++_minmax_0.05_spcl_0.0053_bootstrap_results_5000_iterations.json'
    ...
    # with open(f'__results_revision/{args.dataset}_p_values.json', 'a') as f:
    #     json.dump('connection_sensitive_loss:', f)
