...
class UNetModule(nn.Module):
    ...
    def forward(self, x):
        ...
        return x, cl_features


class RRWNet(nn.Module):
    """Recurrent refinement network for AV3 segmentation."""
    ...
    def forward(self, x):
        ...
        pred_1, cl_features_1 = self.first_u(x)
        predictions.append(pred_1)
        cl_features.append(cl_features_1)

        bv_logits = pred_1[:, 2:3, :, :]
        pred_1 = torch.sigmoid(pred_1)
        bv = pred_1[:, 2:3, :, :]

        pred_2, cl_features_2 = self.second_u(pred_1)
        predictions.append(torch.cat((pred_2, bv_logits), dim=1))
        cl_features.append(cl_features_2)

        for _ in range(self.num_iterations):
            pred_2 = torch.sigmoid(pred_2)
            pred_2 = torch.cat((pred_2, bv), dim=1)
            pred_2, cl_features_2 = self.second_u(pred_2)
            predictions.append(torch.cat((pred_2, bv_logits), dim=1))
            cl_features.append(cl_features_2)

        return img, predictions, cl_features
