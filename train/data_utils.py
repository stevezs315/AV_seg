...
def get_sets(images, num_imgs_val):
    images_per_fold = num_imgs_val
    # print(f'Images per fold: {len(images)}')
    folds = []
    current_fold = {
            'validation': images[: images_per_fold],
            'training': images[images_per_fold: ],
        }
    folds.append(current_fold)
    # print(f'Folds: {folds}')
    return folds
